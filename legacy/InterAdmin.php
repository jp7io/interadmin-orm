...
class InterAdmin extends Record implements InterAdminAbstract
{
    ...
    protected function _deprecatedFindChild($nome_id)
    {
        ...
        if (empty($children[$nome_id])) {
            $nome_id = explode('_', Str::snake($nome_id));
            $nome_id[0] = self::deprecatedPluralPt($nome_id[0]);
            $nome_id = Str::studly(implode('_', $nome_id));
        }
        if (empty($children[$nome_id])) {
            $nome_id = self::deprecatedPluralPt($nome_id);
        }
        return $children[$nome_id];
    }

    /**
     * Portuguese pluralizer preserving the legacy Jp7_Inflector::plural() rules used
     * only by the deprecated magic child-finders above. Retired together with
     * _deprecatedFindChild() once tenant code uses explicit getChildren()/getChildrenTipo().
     *
     * @deprecated
     */
    private static $deprecatedPluralInflections = [
        '/m$/i' => 'ns',                                          // Bem -> Bens
        '/([r|z])$/i' => '\1es',                                  // Paz -> Pazes
    ...
    private static function deprecatedPluralPt($word)
    {
        foreach (self::$deprecatedPluralInflections as $pattern => $replacement) {
