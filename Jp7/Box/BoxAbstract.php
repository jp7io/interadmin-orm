...
');
		...
		<div class="group">
			...
			<div class="group-fields">
				<div class="field">
					...
					<?php echo $this->numericField('imgWidth', 'Largura', '80');
        ?> x
					<?php echo $this->numericField('imgHeight', 'Altura', '60');
        ?> px
				</div>
				<div class="field">
    ...
    public function checkbox($name, $default_value = false)
    {
        ...
        ob_start();
        ?>
		<input type="hidden" value="<?php echo $this->params->$name ? '1' : '0';
        ?>" name="<?php echo $this->id_box;
        ?>[<?php echo $name;
        ?>][]" />
		<input type="checkbox" class="checkbox" <?php echo $this->params->$name ? 'checked="checked"' : '';
        ?> onclick="$(this).prev().val(this.checked ? 1 : 0)" />
		<?php
        return ob_get_clean();
    }
    /**
    ...
    public function numericField($name, $label, $placeholder = '')
    {
        ob_start();
        ?>
		<input type="text" class="numeric textbox" label="<?php echo $label;
