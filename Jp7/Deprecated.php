...
class Jp7_Deprecated
{
    ...
    public static function jp7_index($lang = '')
    {
        ...
        if ($admin_time > $index_time || date('d') != date('d', $index_time)) {
            $publish = true;
        }
        // Redirect
    ...
    public static function wap_toHTML($S)
    {
        ...
        $S = preg_replace('([áàãâäª])', 'a', $S);
        $S = preg_replace('([éèêë&])', 'e', $S);
        $S = preg_replace('([íìîï])', 'i', $S);
        $S = preg_replace('([óòõôöº])', 'o', $S);
    ...
    public static function jp7_print_r($var, $return = false, $hideProtectedVars = false, $varPrefix = '')
    {
        ...
        if ($array) {
            foreach ($array as $key => $value) {
                if ($varPrefix && strpos($key, $varPrefix) !== 0) {
                    continue;
                }
                if ($hideProtectedVars) {
            ...
            if (is_object($var) && $hideProtectedVars) {
                $array = $array[0];
            }
