...
class Jp7_Diff_Xml {
	...
	public static function namespaceSelector( $selected = '', $all = null, $element_name = 'namespace', $label = null ) {
		...
		$ret = Jp7_Diff_Xml::openElement( 'select', array( 'id' => 'namespace', 'name' => $element_name,
			'class' => 'namespaceselector' ) )
			. "\n"
			. implode( "\n", $options )
			. "\n"
			. Jp7_Diff_Xml::closeElement( 'select' );
		if ( !is_null( $label ) ) {
			$ret = Jp7_Diff_Xml::label( $label, $element_name ) . '&nbsp;' . $ret;
		}
		return $ret;
	}
	...
	public static function dateMenu( $year, $month ) {
		...
		return Jp7_Diff_Xml::label( wfMsg( 'year' ), 'year' ) . ' '.
			Jp7_Diff_Xml::input( 'year', 4, $encYear, array('id' => 'year', 'maxlength' => 4) ) . ' '.
			Jp7_Diff_Xml::label( wfMsg( 'month' ), 'month' ) . ' '.
			Jp7_Diff_Xml::monthSelector( $encMonth, -1 );
	}
	...
	public static function languageSelector( $selected, $customisedOnly = true ) {
		...
		foreach( $languages as $code => $name ) {
			$options .= Jp7_Diff_Xml::option( "$code - $name", $code, ($code == $selected) ) . "\n";
		}

		return array(
			Jp7_Diff_Xml::label( wfMsg('yourlanguage'), 'wpUserLanguage' ),
			Jp7_Diff_Xml::tags( 'select',
	...
	public static function listDropDown( $name= '', $list = '', $other = '', $selected = '', $class = '', $tabindex = Null ) {
		...
		return Jp7_Diff_Xml::openElement( 'select', $attribs )
			. "\n"
			. $options
			. "\n"
			. Jp7_Diff_Xml::closeElement( 'select' );
	...
	public static function fieldset( $legend = false, $content = false, $attribs = array() ) {
		$s = Jp7_Diff_Xml::openElement( 'fieldset', $attribs ) . "\n";
		if ( $legend ) {
			$s .= Jp7_Diff_Xml::element( 'legend', null, $legend ) . "\n";
		}
		if ( $content !== false ) {
			$s .= $content . "\n";
			$s .= Jp7_Diff_Xml::closeElement( 'fieldset' ) . "\n";
		}
	...
	public static function isWellFormedXmlFragment( $text ) {
		$html =
			...
			$text .
			'</html>';
		return Jp7_Diff_Xml::isWellFormed( $html );
	}
	...
	*/
	public static function buildForm( $fields, $submitLabel = null ) {
		...
		foreach( $fields as $labelmsg => $input ) {
			$id = "mw-$labelmsg";
			$form .= Jp7_Diff_Xml::openElement( 'tr', array( 'id' => $id ) );
			$form .= Jp7_Diff_Xml::tags( 'td', array('class' => 'mw-label'), wfMsgExt( $labelmsg, array('parseinline') ) );
			$form .= Jp7_Diff_Xml::openElement( 'td', array( 'class' => 'mw-input' ) ) . $input . Jp7_Diff_Xml::closeElement( 'td' );
			$form .= Jp7_Diff_Xml::closeElement( 'tr' );
		}

		if( $submitLabel ) {
			$form .= Jp7_Diff_Xml::openElement( 'tr' );
			$form .= Jp7_Diff_Xml::tags( 'td', array(), '' );
			$form .= Jp7_Diff_Xml::openElement( 'td', array( 'class' => 'mw-submit' ) ) . Jp7_Diff_Xml::submitButton( wfMsg( $submitLabel ) ) . Jp7_Diff_Xml::closeElement( 'td' );
			$form .= Jp7_Diff_Xml::closeElement( 'tr' );
		}
	...
	public static function buildTable( $rows, $attribs = array(), $headers = null ) {
		$s = Jp7_Diff_Xml::openElement( 'table', $attribs );
		if ( is_array( $headers ) ) {
			foreach( $headers as $id => $header ) {
				$attribs = array();
				if ( is_string( $id ) ) $attribs['id'] = $id;
				$s .= Jp7_Diff_Xml::element( 'th', $attribs, $header );
			}
		}
		foreach( $rows as $id => $row ) {
			$attribs = array();
			if ( is_string( $id ) ) $attribs['id'] = $id;
			$s .= Jp7_Diff_Xml::buildTableRow( $attribs, $row );
		}
		$s .= Jp7_Diff_Xml::closeElement( 'table' );
		return $s;
	}
	...
	public static function buildTableRow( $attribs, $cells ) {
		$s = Jp7_Diff_Xml::openElement( 'tr', $attribs );
		foreach( $cells as $id => $cell ) {
			$attribs = array();
			if ( is_string( $id ) ) $attribs['id'] = $id;
			$s .= Jp7_Diff_Xml::element( 'td', $attribs, $cell );
		}
		$s .= Jp7_Diff_Xml::closeElement( 'tr' );
		return $s;
	}
}
